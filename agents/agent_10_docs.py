...
class DocsAgent(BaseAgent):
    ...
    async def _execute(
        ...
        arch_type = str(
            (state.get("service_graph") or {}).get("architecture_type", "Service")
        )
        changelog = (
            f"# Changelog\n\n"
            f"## [0.1.0] — {datetime.now(tz=timezone.utc).strftime('%Y-%m-%d')}\n\n"
            f"### Added\n"
    ...
    async def _save_project_context_graph(
        ...
        if not services:
            services = [ServiceNode(
                ...
                exposes=[], depends_on=[], owns_data=True, database=None,
            )]
        ...
        graph = ProjectContextGraph(
            ...
            api_contracts=(
                ...
                if state.get("deployment_url") else []
            ),
