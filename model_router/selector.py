...
class ModelSelector:
    ...
    def select(self, agent: str) -> BaseLLMAdapter:
        ...
        if model is None:
            ...
            raise ForgeSDLCError(
                f"Agent '{agent}' has no internal LLM. It must delegate via ToolRouter."
            )
    ...
    def _build_adapter(self, model: str) -> BaseLLMAdapter:
        ...
        if model in RESPONSES_API_MODELS:
            from model_router.adapters.openai_reasoning_adapter import (
                OpenAIReasoningAdapter,  # noqa: PLC0415
            )
        ...
        if model.startswith("gemini-"):
            from model_router.adapters.gemini_adapter import (
                GeminiAdapter,  # noqa: PLC0415
            )
        ...
        if model.startswith("devstral") or model.startswith("ollama/"):
            from model_router.adapters.ollama_adapter import (
                OllamaAdapter,  # noqa: PLC0415
            )

            return OllamaAdapter(model=model)

        if "azure" in model:
            from model_router.adapters.azure_adapter import (
                AzureOpenAIAdapter,  # noqa: PLC0415
            )
