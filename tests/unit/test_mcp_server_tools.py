...
Updated for v1.1.0:
- Infrastructure function renamed to _build_infrastructure_shared() in all tools
...
def _make_infra_tuple(n: int = 7) -> tuple:
...
async def test_gather_requirements_input_validation_empty_prompt() -> None:
    """Empty prompt must return error without touching infrastructure."""
    ctx = _make_mock_ctx()
    result = await gather_requirements(prompt="", project_id="p1", ctx=ctx)
    assert result["status"] == "error"
    assert "empty" in str(result["error"]).lower()


@pytest.mark.asyncio
async def test_gather_requirements_input_validation_empty_project_id() -> None:
    ctx = _make_mock_ctx()
    result = await gather_requirements(prompt="build an API", project_id="", ctx=ctx)
    assert result["status"] == "error"
...
async def test_gather_requirements_hitl_gate_correct_keys() -> None:
    ...
    async def agent0_sets_service_graph(state: dict) -> dict:
        state["service_graph"] = {"architecture_type": "monolith", "services": []}
        state["human_confirmation"] = ""
        return state

    async def agent1_sets_prd(state: dict) -> dict:
        state["prd"] = "# PRD\n## User Stories\n- As a user..."
        state["human_confirmation"] = ""
        return state

    async def agent2_sets_adr(state: dict) -> dict:
        state["adr"] = "# ADR-001: FastAPI + PostgreSQL\n## Decision\nFastAPI"
        state["human_confirmation"] = ""
        return state
    ...
    with patch("mcp_server.tools.requirements_tool._build_infrastructure"):  # noqa: SIM117
        with patch("mcp_server.tools.requirements_tool._build_agents") as mock_build:
            ...
            mock_a0.run = AsyncMock(side_effect=agent0_sets_service_graph)
            mock_a1 = MagicMock(spec=RequirementsAgent)
            mock_a1.run = AsyncMock(side_effect=agent1_sets_prd)
            mock_a2 = MagicMock(spec=TechStackAgent)
            mock_a2.run = AsyncMock(side_effect=agent2_sets_adr)
    ...
    # With all three agents setting correct keys, should complete
    assert result["status"] in ("complete", "awaiting_confirmation")
    assert "project_id" in result


@pytest.mark.asyncio
async def test_gather_requirements_awaiting_after_agent0() -> None:
    """Agent 0 runs but returns no service_graph → still awaiting."""
    ctx = _make_mock_ctx()

    async def agent0_no_output(state: dict) -> dict:
        # Agent 0 doesn't set service_graph (simulate incomplete run)
        state["human_confirmation"] = ""
        return state
    ...
    with patch("mcp_server.tools.requirements_tool._build_infrastructure"):  # noqa: SIM117
        with patch("mcp_server.tools.requirements_tool._build_agents") as mock_build:
            ...
            mock_a0 = MagicMock(spec=ServiceDecompositionAgent)
            mock_a0.run = AsyncMock(side_effect=agent0_no_output)
            mock_a1 = MagicMock(spec=RequirementsAgent)
            mock_a1.run = AsyncMock(return_value={})
            mock_a2 = MagicMock(spec=TechStackAgent)
            mock_a2.run = AsyncMock(return_value={})
            mock_build.return_value = (mock_a0, mock_a1, mock_a2)
...
async def test_recall_context_returns_valid_dict() -> None:
    ...
    # Build a real MemoryContext object — the mock must return this, not a dict
    ...
    with patch("mcp_server.tools.memory_tool.MemoryContextBuilder") as mock_builder_cls:
        mock_builder_cls.return_value.build = AsyncMock(return_value=mock_memory_context)
        result = await recall_context(query="what stack?", project_id="p1", ctx=ctx)
...
async def test_save_decision_returns_valid_dict() -> None:
    ...
    with patch("mcp_server.tools.memory_tool.OrgMemory") as mock_org:
        ...
        result = await save_decision(
            decision="use postgres", rationale="scale", project_id="p1", ctx=ctx
        )
...
async def test_route_code_generation_returns_valid_dict() -> None:
    ...
    async def fake_a5(state: dict) -> dict:
...
async def test_generate_cicd_returns_valid_dict() -> None:
    ...
    async def fake_a7(state: dict) -> dict:
