...
async def test_agent_11_uses_gemini_via_long_context_router() -> None:
    ...
    _agent = IntegrationAgent(name="agent_11_integration", **kwargs)  # used implicitly
    ...
    assert isinstance(adapter.inner, GeminiAdapter), (
        f"Expected GeminiAdapter (inner) for 150K tokens, got {type(adapter.inner).__name__}. "
