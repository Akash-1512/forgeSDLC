...
async def track_progress(project_id: str) -> dict[str, object]:
        ...
        try:
            with sqlite3.connect(str(checkpoint_db), check_same_thread=False) as conn:
                ...
                if existing and existing.get("channel_values"):
                    ...
                    if cv.get("adr"):         stages_complete.append("stack_selection")
                    if cv.get("prd"):         stages_complete.append("requirements")
                    if cv.get("service_graph"): stages_complete.append("decomposition")
                    if cv.get("rfc"):         stages_complete.append("architecture")
                    if cv.get("generated_files"): stages_complete.append("code_generation")
                    if cv.get("security_gate"): stages_complete.append("security_scan")
                    if cv.get("ci_pipeline_url"): stages_complete.append("cicd")
                    if cv.get("deployment_url"): stages_complete.append("deployment")
                    if cv.get("monitoring_config"): stages_complete.append("monitoring")
                    if cv.get("project_context_graph"): stages_complete.append("documentation")
    ...
    token_summary: dict[str, object] = {"total_calls": 0, "by_agent": {}, "by_model": {}}
    if token_records:
        try:
    ...
    all_stages = [
        "decomposition", "requirements", "stack_selection", "architecture",
        "code_generation", "security_scan", "cicd", "deployment",
